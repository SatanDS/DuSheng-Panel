...
- 管理员/用户登录，认证方式为 JWT。
- 租户、租户管理员、设备组、节点、逻辑隧道、IEPL 线路资产、A/Z 端点、转发规则、线路探测、限速、流量采样、节点事件、审计日志、安装令牌、协议策略。
...
超级管理员可创建租户并把 `tenant_admin` 用户绑定到租户。租户管理员只能管理本租户普通用户、转发规则和用量，不能创建管理员、跨租户访问数据或自行取得未授权线路。

租户使用逻辑线路前必须配置“租户线路授权”。租户账号的只读线路列表只返回已授权线路；授权可限制该线路下的规则数量和入口端口范围，缩小授权时 API 会拒绝任何无法容纳现有规则的变更。规则级协议策略只能由超级管理员设置，租户管理员和普通用户编辑规则时会保留既有策略，不能覆盖线路或入口组的合规策略。端口由数据库租约按“入口设备组 + Bind IP + transport + port”原子占用，`tcp_udp` 会同时占用 TCP 与 UDP，避免不同逻辑线路在同一入口节点撞端口。规则批量导入先预检，再在单个事务内提交，任一规则失败时整批回滚。

流量上报同时计入用户和租户当前周期，`reportId` 保证重试幂等。任一层配额耗尽都会停止下发相关规则；租户配额重置后，仍耗尽个人配额的规则不会被错误恢复。租户限速按租户内所有规则共享连接数、来源 IP、上传和下载带宽，规则/线路/用户级限制仍按最严格值叠加执行。

## Prometheus 监控

API 在容器内的 `/metrics` 暴露低基数 HTTP、节点、配置回执、规则、租户状态、租户配额封禁、租户线路授权、物理线路、探测和违规指标；Caddy 默认不对公网转发该路径。Agent 默认在 `127.0.0.1:19090` 暴露 listener 生命周期、连接、配置租约、DPI、流量缓冲和线路探测指标。
