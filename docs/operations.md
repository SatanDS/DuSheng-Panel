...
用户线路授权数量使用低基数指标 `dusheng_panel_user_tunnel_grants`。兼容租户监控继续提供 `dusheng_panel_tenants{status}`、`dusheng_panel_tenant_accounted_bytes`、`dusheng_panel_tenant_quota_blocked` 和 `dusheng_panel_tenant_tunnel_grants`。不要把 tenant ID、user ID 或 rule ID 添加为 Prometheus label；具体明细通过面板分页接口查询。

## 用户与线路授权变更

默认从用户页选择普通用户并配置线路授权。用户存在任意直接授权时，只能使用直接授权集合；没有直接授权且仍归属旧租户时，才回退到租户线路授权。修改授权会提升关联节点 desired revision，Agent 下一轮同步会应用结果。配置租约仍是离线节点的最终保护：节点长时间无法同步时，旧 listener 会在租约过期后关闭。

缩小用户授权端口范围或规则数上限前，先检查该用户在对应线路上的现有规则。API 会拒绝排除现有监听端口、低于现有规则数、移动或删除仍被规则引用的授权。
